...
if makefolder and isfolder and writefile and isfile then
	pcall(function()
		...
		if IsOnMobile then writefile("HIS/.nomedia", "") end
	end)
end
...
ColorsButton = Instance.new("TextButton")
...
SettingsButton.BackgroundColor3 = Color3.fromRGB(46, 46, 47)
...
ReferenceButton.BackgroundColor3 = Color3.fromRGB(46, 46, 47)
ReferenceButton.BorderSizePixel = 0
ReferenceButton.Position = UDim2.new(0, 212, 0, 2)
ReferenceButton.Size = UDim2.new(0, 16, 0, 16)
ReferenceButton.Font = Enum.Font.SourceSans
ReferenceButton.Text = "?"
ReferenceButton.TextColor3 = Color3.new(1,1,1)
ReferenceButton.TextSize = 14
...
Credits.Text = "HitInfinityScript"
...
About.TextSize = 14
About.Text = "Plugins are .hins files and should be located in the 'HIS/Plugins' folder of your exploit."
...
PluginsHint.Text = "Download plugins from the HIS community"
...
eventEditor = (function()
	...
	local main = create({
		...
		{5,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image="",Parent={4},Position=UDim2.new(0,5,0,5),Size=UDim2.new(0,10,0,10),ZIndex=10,}},
		...
		{53,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image="",Parent={52},Position=UDim2.new(0,2,0,2),Size=UDim2.new(0,16,0,16),ZIndex=10,}},
	})
	main.Name = randomString()
...
reference = (function()
	local main = create({
		...
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="You can join the Discord server to get support with HIS, and read up on more documentation such as the Plugin API.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
...
function saves()
	if writefileExploit() and readfileExploit() and jsonAttempts < 10 then
		local readSuccess, out = readfile("HIS_FE.his", true)
		if readSuccess then
			if out ~= nil and tostring(out):gsub("%s", "") ~= "" then
				...
				if not success then
					...
					writefile("HIS_FE.his", defaults, true)
					wait()
					saves()
				end
			else
				writefile("HIS_FE.his", defaults, true)
				wait()
				local dReadSuccess, dOut = readfile("HIS_FE.his", true)
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("HIS_FE.his", HttpService:JSONEncode(update))
	end
end
...
if type(PluginsTable) == "table" then
	for i = #PluginsTable, 1, -1 do
		if string.sub(PluginsTable[i], -4) ~= ".hins" then
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- HitInfinityScript Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
function selectPart()
	...
	local function HighlightPart()
		if selected.Adornee ~= HISMouse.Target then
			selectionBox.Adornee = HISMouse.Target
		else
			selectionBox.Adornee = nil
		end
	end
	ActivateHighlight = HISMouse.Move:Connect(HighlightPart)
	local function SelectPart()
		if HISMouse.Target ~= nil then
			selected.Adornee = HISMouse.Target
			Path.Text = getHierarchy(HISMouse.Target)
		end
	end
	ClickSelect = HISMouse.Button1Down:Connect(SelectPart)
end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the HIS Discord server.'}
...
CMDs[#CMDs + 1] = {NAME = 'hideiy', DESC = 'Hides the main GUI'}
CMDs[#CMDs + 1] = {NAME = 'showiy / unhideiy', DESC = 'Shows GUI again'}
CMDs[#CMDs + 1] = {NAME = 'keepiy', DESC = 'Auto execute HIS when you teleport through servers'}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'addplugin / plugin [name]', DESC = 'Add a plugin via command (supports .hins)'}
...
CMDs[#CMDs + 1] = {NAME = 'addallplugins / loadallplugins', DESC = 'Adds all available .hins plugins from the HIS/Plugins folder'}
...
function checkTT()
	local t
	local guisAtPosition = COREGUI:GetGuiObjectsAtPosition(HISMouse.X, HISMouse.Y)
	...
	if t ~= nil and t:GetAttribute("Title") ~= nil then
		...
		local xP
		local yP
		if HISMouse.X > 200 then
			xP = x - 201
		else
			xP = x + 21
		end
		if HISMouse.Y > (HISMouse.ViewSizeY-96) then
			yP = y - 97
		else
			yP = y
		end
			...
			if cmd then
				...
				if infTimes then
					while lastBreakTime < cmdStartTime do
						...
						if not success and _G.HIS_DEBUG then
...
local MousePositionToVector2 = function()
	return Vector2.new(HISMouse.X, HISMouse.Y)
end
...
local function clicktpFunc()
	pcall(function()
		...
		local hitPosition = HISMouse.Hit.Position
...
HISMouse.Button1Down:Connect(function()
	for i,v in pairs(binds) do
		...
		elseif v.COMMAND == 'clickdel' then
			...
			if input == 'RightClick' and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
				pcall(function() HISMouse.Target:Destroy() end)
...
function addPlugin(name)
	if name:lower() == 'plugin file name' or name:lower() == 'his_fe.his' or name == 'his_fe' then
		notify('Plugin Error','Please enter a valid plugin')
	else
		local file
		local fileName
		if name:sub(-5) == '.hins' then
			pcall(function() file = readfile("HIS/Plugins/" .. name) end)
			fileName = name
		else
			pcall(function() file = readfile("HIS/Plugins/" .. name .. '.hins') end)
			fileName = name .. '.hins'
		end
		if file then
			...
			notify('Plugin Error','Cannot locate file "'..fileName..'". Is the file in the HIS/Plugins folder?')
		end
	end
end

function deletePlugin(name)
	local pName = name .. '.hins'
	if name:sub(-5) == '.hins' then
		pName = name
	end
	for i = #cmds,1,-1 do
...
function LoadPlugin(val,startup)
	...
	function CatchedPluginLoad()
		plugin = loadfile("HIS/Plugins/" .. val)()
	end
...
addcmd('keepiy', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = true
		notify('KeepIY','HitInfinityScript will now run after you teleport')
		updatesaves()
	else
...
addcmd('unkeepiy', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = false
		notify('KeepIY','HitInfinityScript will no longer run after you teleport')
		updatesaves()
	else
		notify('Incompatible Exploit','Your exploit does not support this command (missing queue_on_teleport)')
	end
...
function deleteGuisAtPos()
	pcall(function()
		local guisAtPosition = PlayerGui:GetGuiObjectsAtPosition(HISMouse.X, HISMouse.Y)
		for _, gui in pairs(guisAtPosition) do
...
addcmd('hideiy',{},function(args, speaker)
	...
	minimizeNum = 0
	minimizeHolder()
	if not (args[1] and tostring(args[1]) == 'nonotify') then notify('HIS Hidden','You can press the prefix key to access the command bar') end
end)
...
addcmd('hovername',{},function(args, speaker)
	...
	nbUpdateFunc = HISMouse.Move:Connect(updateNameBox)
end)
...
addcmd("addallplugins", {"loadallplugins"}, function(args, speaker)
	...
	for _, filePath in ipairs(listfiles("HIS/Plugins")) do
		local fileName = filePath:match("([^/\\]+)%.hins)$")
		if fileName and not isfolder(filePath) and not table.find(PluginsTable, fileName) then
			addPlugin(fileName)
		end
	end
end)
